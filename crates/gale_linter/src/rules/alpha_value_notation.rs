    ...
    match value {
      ...
      serde_json::Value::Array(arr) => {
        ...
        if let Some(secondary) = arr.get(1)
          && let Some(props) = secondary.get("exceptProperties").and_then(|v| v.as_array())
        {
          for item in props {
...
mod tests {
  ...
  fn number_mode_reports_percentage_opacity() {
    let opts = serde_json::json!(["number"]);
    let d = AlphaValueNotation.check(&style_with_decl("opacity", "50%"), &ctx_with_options(&opts));
    assert_eq!(d.len(), 1);
  }
  ...
  fn number_mode_allows_decimal_opacity() {
    let opts = serde_json::json!(["number"]);
    let d = AlphaValueNotation.check(&style_with_decl("opacity", "0.5"), &ctx_with_options(&opts));
    assert!(d.is_empty());
  }
