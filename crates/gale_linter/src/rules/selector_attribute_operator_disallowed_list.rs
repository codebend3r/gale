...
impl Rule for SelectorAttributeOperatorDisallowedList {
  ...
  /// Flags attribute operators named in the configured disallow list.
  ...
  while i < len {
    if chars[i] == '[' {
      ...
      while i < len && chars[i] != ']' {
        ...
        if i + 1 < len && chars[i + 1] == '=' && matches!(chars[i], '~' | '|' | '^' | '$' | '*') {
...
mod tests {
  ...
  fn allows_operator_not_in_list() {
    let ctx = ctx_with_options(serde_json::json!(["*="]));
    let d =
      SelectorAttributeOperatorDisallowedList.check(&style_with_selector("a[class=\"foo\"]"), &ctx);
    assert!(d.is_empty());
  }
